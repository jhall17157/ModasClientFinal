$(function () {
  ...
  function showTableBody(e) {
    ...
    for (i = 0; i < e.length; i++) {
      ...
      html += "<td>" + e[i].loc + "</td>";
      html += "<td>" + "<button class='deleteEvent'>Delete</button>" + "</td>"
      html += "</tr> ";
    }
    $('tbody').html(html);
  }
  ...
  //delete button
  $('tbody').on('click', '.deleteEvent', function () {
    var checked;
    // AJAX to update database delete
    $.ajax({
      headers: { "Content-Type": "application/json" },
      url: "https://modasapi.azurewebsites.net/api/event/" + $(this).data('id'),
      type: 'DELETE',
      success: function () {
        // Toast
        toast("Update Complete", "Event " + (checked ? "added." : "removed."), "far fa-edit");
      },
      error: function (jqXHR, textStatus, errorThrown) {
        // log the error to the console
        console.log("The following error occured: " + jqXHR.status, errorThrown);
